...
function isAddressChanged(file: NoteFile, address: string) {
  return getAddress(file.key) !== address;
}

function matchesExistingAddress(address: string, existingFiles: NoteFile[]) {
  return existingFiles.some((file) => getAddress(file.key) === address);
}

function getAddress(fileKey: string) {
